...
- **（Mac）log 完順手補 story graph**——此刻你脈絡全在手上，補圖
  近乎零成本：若 `<vault>/…/Canvas/<專案>·脈絡心智圖.graph.json` 存在，
  在其尾端 append 一個節點（label/text＝本次主題與結論、
  `sources=[<新 log 卡 id>]`、anchor 視蒸餾去向、必要時開新幕標
  `stage`）＋承接邊（這次 log 的前情提要引用誰，邊就從誰來），然後
  重渲染 story canvas（指令與 schema 見 **project-card-canvas** skill）。
  graph 不存在＝該專案還沒建敘事圖，跳過（建圖是 canvas skill 的事）；
  漏補也無妨——下次 render 的紅色待補橫幅會抓到。
...
- **（cluster→Mac 自動收斂）**——`hb` transport 的 log 卡＋時間線行都
  ...
  **必要時 repair_chain --seal → pinpoint project-card-repair →
  ...
  任一步失敗／note-sync 有 conflict 都保留事件重試。story 模式只會沿用
  現有 graph 重渲染並顯示 coverage 缺口——語義節點仍須 agent 補寫。
