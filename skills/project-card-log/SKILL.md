...
- **（實驗）脈絡心智圖**——三種拆解單位（需 backend=both）：
  ```bash
  python3 <plugin 的 skills/project-card-canvas 目錄>/context_mindmap.py --card <ENTRY_ID>   # logs 模式
  python3 <同目錄>/context_mindmap.py --card <ENTRY_ID> --mode chain  # chain 模式
  python3 <同目錄>/context_mindmap.py --card <ENTRY_ID> --mode story \
      --graph <vault>/…/<專案>·脈絡心智圖.graph.json                        # story 模式
  ```
  **logs**（預設）：每張 self-contained log 卡拆成 root（軸卡）→ hub
