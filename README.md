...
| `project-card-canvas` | The visual views of a project chain as generated JSON Canvas: a git-graph **timeline** (entry = HEAD, log cards = commits, colored by distillation state or origin machine) and a **context mind map** with three modes — per-log-card subtrees, chain-body sections, or the research **story** as a staged multi-row DAG (the agent reads the chain and authors a narrative graph of ideas→experiments→results→pivots; the script lays it out deterministically) |
