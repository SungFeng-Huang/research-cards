...
**訓練進度儀表**（選配）——把訓練 log 變成即時 SVG 曲線頁：

```bash
python3 …/campaign.py progress-init --dir runs/auto_research  # 生成 progress.json 模板
# 模板的 _doc 欄逐項教學；填三件事：
#   log_glob   訓練 log 位置（如 slurm_logs/*.out；支援 **，限 project 內）
#   step_re    步數行的 regex（必含 (?P<step>…) named group）
#   kv_re      指標 key=value 的 regex——log 格式是你的，規則放設定不寫死
#   charts     圖表宣告：每圖一 y 軸、series ≤4、refs 畫參考線
python3 …/campaign.py progress --dir runs/auto_research       # → campaign-progress.html
```

stat tiles＋SVG 曲線（crosshair tooltip、參考線直標、雙 series legend）＋
無滑鼠 fallback 的取樣資料表＋ladder/ledger/job 鏈表，與 report 互連。
解析細節都替你想好了：搶佔重跑的重疊步以較新 job 為準（逐 key 合併，
雜訊行不清空資料）、曲線抽稀 ≤700 點、指標存 6 位有效數字（lr=5e-06
不會變 0）、多 GB log 串流讀取；輸出對相同輸入位元級確定（無牆鐘時間
戳）。`scheduler:"slurm"` 可 best-effort 顯示 squeue 狀態與 ETA（指令
寫死不吃設定——進版控的 progress.json 拿不到 shell）；預設 `none` 全離線。

之後頁面**自動跟著 campaign 演進**：每個 job 收尾（per-job step 7）regen
report（與 progress）、連同 ledger 一起 commit＋push 到部署分支，CI 的
paths 過濾就觸發 Pages 重新部署——不需要另外的排程器（實驗分支的 push
刻意不觸發）。報告頁含 ladder 狀態徽章、ledger 全表（顯著性
徽章＋playbook 引用）、BLOCKED 橫幅；輸出無時間戳（確定性，發佈時間交給
git 歷史）。領域特定 demo 頁（音檔 A/B 對聽、樣本展示等）自行加在
發佈目錄，同一條 CI 一起發佈。
