...
# 填 log_glob（如 slurm_logs/*.out）、step_re/kv_re（你的 log 格式）、charts
python3 …/campaign.py progress --dir runs/auto_research       # → campaign-progress.html
```

stat tiles＋2px 曲線（crosshair tooltip、參考線、搶佔重跑以較新 job 為準、
抽稀 ≤700 點）＋資料表 fallback＋ladder/ledger/job 鏈，與 report 互連；
`scheduler:"slurm"` 可 best-effort 顯示 squeue 狀態與 ETA。
