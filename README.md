...
| `project-card-canvas` | The visual views of a project chain as generated JSON Canvas: a git-graph **timeline** (log cards = a commit column, entry = the HEAD pointer riding beside the newest distilled log with the 📎 backlog stacking above it — distillation state is topology; colors default to the origin machine) and a **context mind map** with three modes — per-log-card subtrees, chain-body sections, or the research **story** as a staged multi-row DAG (the agent reads the chain and authors a narrative graph of ideas→experiments→results→pivots; the script lays it out deterministically) |
...
| "Draw / refresh this project's canvases" — git-graph timeline + context mind map; for the story view the agent reads the chain, authors the narrative graph, and the coverage audit reports what the map is still missing | `/research-cards:project-card-canvas`(inside the project repo session) |
