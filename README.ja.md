...
ノートバックエンドは **Heptabase** と **Obsidian** の 2 つ（双方向同期可能）、
AI agent は **Claude Code** と **Codex** の 2 つをサポートし、どの組み合わせでも使えます。できることは：
...
[インストール](#インストール) ·
[設定](#設定) ·
[クイックスタート（Obsidian）](#クイックスタート-a--obsidian-をゼロから) ·
...
| `backend: obsidian` / `both` | **Obsidian vault** 1 つ（iCloud に置く場合、ターミナルに**フルディスクアクセス権限**が必要） |
...
## クイックスタート A — Obsidian をゼロから

Heptabase は不要です。10 分で論文パイプラインを立ち上げます：

1. **インストール**：plugin（上記参照）＋ `pip install pyyaml`
   （図版が欲しければさらに `pymupdf` と `librsvg`）。
2. **vault を作成**（場所は自由。iCloud でも構いませんが、ターミナルにフルディスクアクセス権限が必要です）。
...
5. **関連論文が数本たまったら、構造を育てます**：
   ...
   ナレッジマップは、あなた自身がレイアウトする 1 枚の Obsidian Canvas です。

## クイックスタート B — Heptabase / both

同じ流れですが、最初に id を調べるステップが 1 つ加わります：

1. **Heptabase デスクトップ版**と `heptabase` CLI（≥ 0.4.0）をインストールします。
2. Heptabase で自分の tags（例：`study/paper`、`study/overview`、
   `project`）とプロパティを作り、それから id を収集します：

   ```bash
   heptabase tag list                      # tag ids
   heptabase tag properties <tagId>       # プロパティ UUID
   ```

3. config の `heptabase` セクションを記入します（workspace id、tag id つきの
   `collections`、`props`、`graph`）。`backend` は `heptabase` に——または
   `both` にすると[双方向 vault 同期](#heptabase--obsidian-同期)も手に入ります
   （`obsidian` セクションの記入も必要）。
4. クイックスタート A のステップ 4 に従って最初のカードをクリップします。
...
`backend: both` は Heptabase を正とし、カードの collection を vault へミラーリングします——vault
側の編集も書き戻されます：

```bash
cd skills/obsidian-sync
python3 sync.py --dry-run     # プレビュー
python3 sync.py               # 本実行——出力される JSON レポートを読むこと
python3 verify.py             # vault の整合性チェック。CLEAN が期待値
```

- **Collections は config 駆動**：`tag_id` を記入した
  `heptabase.collections.<key>` エントリはすべて `obsidian.folders.<key>` へミラーリングされます
  （オプションの `filter` でプロパティによる絞り込みが可能——例：papers → Source Type=alphaXiv）。
  典型的な組み合わせ：papers → `Papers/`、overviews → `Overviews/`、projects →
  `Projects/`。
- **フォワード**（Heptabase → vault）：増分——変更のないカードは `lastEditedTime`＋
  コンテンツハッシュでスキップ。タイトルを変えるとファイル名の変更と wikilinks の書き換えが連動します。
- **書き戻し**（vault → Heptabase）：キャッシュ済みの ProseMirror スナップショットと突き合わせて**ブロック単位**で
  マージ。触っていないブロックはそのまま残ります。方言で無損失に表現できないもの（highlight 埋め込み、
  複雑なテーブル、カスタム画像サイズ）→ カード丸ごと**コンフリクト報告、書き戻さない**——1 枚のカードは全か無かです。
- **取り込み（adoption）**：管理対象フォルダに直接新規作成した `.md` は、新しい Heptabase カードになり、
  その collection の tag が付きます。
- **コンフリクト台帳**：本実行のたびに vault ルートの `Sync Conflicts.md` を再生成します
  （未解決＋自動アーカイブされた解決済み）。このファイルは手で編集しないでください。
- **プロパティ**：スナップショットつきの 3-way 同期——片側だけ変更 → 伝播。両側とも変更 → コンフリクト報告、書き込みなし。

round-trip を安全にする markdown 方言：文字色は `<span style="color:…">`、
下線は `<u>`、Heptabase の toggle は `- ⏵ ` プレフィックス（あえて checkbox を使いません——クリック
できるものがなければマーカーが壊されることもありません）、同一カード内アンカーは Obsidian の block reference
（`[[#^id]]`）、カードリンクは `[[wikilinks]]`。詳細は
`skills/obsidian-sync/SKILL.md` を参照。
