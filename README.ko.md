...
**Heptabase**와 **Obsidian** 두 노트 백엔드(양방향 동기화 가능),
**Claude Code**와 **Codex** 두 AI agent를 지원하며, 어떤 조합이든 됩니다. 이 플러그인이 해 주는 일:
...
[설치](#설치) ·
[설정](#설정) ·
[빠른 시작(Obsidian)](#빠른-시작-a--obsidian-처음부터) ·
...
| `backend: obsidian` / `both` | **Obsidian vault** 하나(iCloud에 둘 경우 터미널에 **전체 디스크 접근 권한** 필요) |
...
## 빠른 시작 A — Obsidian, 처음부터

Heptabase가 필요 없습니다. 10분 만에 논문 파이프라인을 세웁니다:

1. **설치** — plugin(위 참고) + `pip install pyyaml`
   (그림까지 원하면 `pymupdf`와 `librsvg` 추가).
2. **vault 생성**(위치는 자유; iCloud도 괜찮지만 터미널에 전체 디스크 접근 권한이 있어야 합니다).
...
5. **관련 논문이 몇 편 쌓이면, 구조를 키웁니다**:
   ...
   지식 맵은 여러분이 직접 레이아웃하는 Obsidian Canvas 한 장입니다.

## 빠른 시작 B — Heptabase / both

같은 흐름에, 앞에 id 조회 한 단계가 더 붙습니다:

1. **Heptabase 데스크톱 앱**과 `heptabase` CLI(≥ 0.4.0)를 설치합니다.
2. Heptabase에서 여러분의 tags(예: `study/paper`, `study/overview`,
   `project`)와 속성을 만든 뒤 id를 수집합니다:

   ```bash
   heptabase tag list                      # tag ids
   heptabase tag properties <tagId>       # 속성 UUID
   ```

3. config의 `heptabase` 섹션을 채웁니다(workspace id, tag id가 들어간
   `collections`, `props`, `graph`). `backend`는 `heptabase`로 — 또는
   `both`로 설정하면 [양방향 vault 동기화](#heptabase--obsidian-동기화)를
   덤으로 얻습니다(`obsidian` 섹션도 채워야 합니다).
4. 빠른 시작 A의 4단계대로 첫 카드를 클리핑합니다.
...
`backend: both`는 Heptabase를 기준으로 카드 collection을 vault로 미러링합니다 — vault
쪽의 편집도 다시 라이트백됩니다:

```bash
cd skills/obsidian-sync
python3 sync.py --dry-run     # 미리보기
python3 sync.py               # 실제 실행 — 출력된 JSON 리포트를 읽으세요
python3 verify.py             # vault 무결성 검사; CLEAN이 기대값
```

- **Collections는 config가 구동합니다**: `tag_id`가 채워진
  `heptabase.collections.<key>` 항목마다 `obsidian.folders.<key>`로 미러링됩니다
  (선택형 `filter`로 속성 필터링 가능 — 예: papers → Source Type=alphaXiv).
  전형적인 조합: papers → `Papers/`, overviews → `Overviews/`, projects →
  `Projects/`.
- **정방향**(Heptabase → vault): 증분 방식 — 바뀌지 않은 카드는 `lastEditedTime` +
  콘텐츠 해시로 건너뜁니다. 제목을 바꾸면 파일명도 연동해 바뀌고 wikilinks도 고쳐 씁니다.
- **라이트백**(vault → Heptabase): 캐시된 ProseMirror 스냅샷과 대조해 **블록 단위**
  병합을 합니다; 손대지 않은 블록은 원형 그대로입니다. 방언이 무손실로 표현할 수
  없는 것(highlight 임베드, 복잡한 표, 커스텀 그림 크기) → 카드 전체를
  **충돌로 보고하고 라이트백하지 않습니다** — 한 카드는 전부 아니면 전무입니다.
- **입양**: 관리 폴더에 직접 새로 만든 `.md`는 새 Heptabase 카드가 되고 해당
  collection의 tag가 붙습니다.
- **충돌 원장**: 실제 실행마다 vault 루트의 `Sync Conflicts.md`를 다시 생성합니다
  (미해결 + 자동 보관된 해결 완료). 이 파일은 손으로 고치지 마세요.
- **속성**: 스냅샷 기반 3-way 동기화 — 한쪽만 변경 → 전파; 양쪽 다 변경 → 충돌로 보고하고 쓰지 않습니다.

round-trip을 안전하게 만드는 markdown 방언: 글자 색은 `<span style="color:…">`,
밑줄은 `<u>`, Heptabase toggle은 `- ⏵ ` 접두어(일부러 checkbox를 쓰지 않습니다 —
클릭할 수 있는 게 없으면 마커가 망가질 일도 없습니다), 같은 카드 내 앵커는
Obsidian block reference(`[[#^id]]`), 카드 링크는 `[[wikilinks]]`. 세부 사항은
`skills/obsidian-sync/SKILL.md` 참고.
