...
| `hackmd-sync` | 選んだ collection を **HackMD** へ一方向・増分ミラーリング（共有／公開）：本物のノート間リンク、HackMD 側の変更検知（編集されたノートはコンフリクトとして報告され、決して上書きされません）、宣言的な閲覧権限 |
...
- **Heptabase**：Heptabase で執筆する（`backend: heptabase`）か、`backend: both` で
  ...
  プロパティの三方向同期付き。
- **HackMD**（公開用で、執筆用ではありません）：`hackmd-sync` が選んだ collection を、
  本物のノート間リンクつきの HackMD ノートとして一方向でミラーリングします——
  オーバービューを共同研究者と共有するために作られています。
