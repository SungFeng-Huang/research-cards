...
[クイックスタート（ノートアプリ）](#クイックスタート-b--ノートアプリを使う) ·
[日常の使い方](#日常の使い方) ·
...
> [Note App Backends](https://github.com/SungFeng-Huang/research-cards/wiki/Note-App-Backends-ja)（Heptabase／Obsidian のセットアップと双方向同期）。
...
| `project-card-canvas` | プロジェクトチェーンの視覚ビュー（生成型 JSON Canvas）：git-graph 式**タイムライン**（log カード＝commit 縦列、entry＝HEAD ポインタが最新の蒸留済み log の横に付き、📎 バックログはその上に積まれる——蒸留状態はトポロジーで表現、色はデフォルトで実行マシン別）と**コンテキストマインドマップ**の 3 モード——log カードごとのサブツリー、チェーン本文の H2/H3 構造、または研究の**ストーリー**を幕ごとの複数行 DAG で（agent がチェーンを読んで「アイデア→実験→結果→転換」の物語グラフを執筆し、script が決定的にレイアウト） |
...
は研究プロジェクトの 3 点セット（log／merge／campaign）をカバーします。`bib-export` と `note-sync` は方向スイッチの影響を受けません
...
| `backends` に `"heptabase"` を含む | macOS＋**Heptabase デスクトップ版**＋`heptabase` CLI **≥ 0.4.0**（ローカル API `127.0.0.1:21210`） |
| `backends` に `"local"` を含む（デフォルト） | **.md ファイルのフォルダが 1 つあれば十分**——どんなディレクトリでも OK。**Obsidian vault** として開くのはオプションの仕上げです（iCloud の vault には**フルディスクアクセス権限**が必要） |
| HackMD ミラーリング（`note-sync --mode hackmd`） | `npm install -g @hackmd/hackmd-cli`＋一度だけの `hackmd-cli login`（API トークンは hackmd.io → Settings → API から取得。plugin の config には決して保存されません） |
...
最小構成の config を書き（純粋 .md モードに必要なのは `local.vault` だけ）、
...
3. **設定**——最小構成の `~/.config/research-cards/config.json`
   （`backends` のデフォルトは `["local"]`——この純粋 .md モード——なので、書かなくても構いません）：
   ...
   （`vault` はただのあなたのフォルダです。このセクションは改名前の
   旧名 `obsidian` でも動きます。）
...
5. **関連論文が数本たまったら、構造を育てます**：
   ...
   3. hub を config の `local.graph.hubs` に登録し、次を実行します：
...
## クイックスタート B — ノートアプリを使う

同じフォルダ——そして同じパイプライン——は、クイックスタート A の前でも後でも、
いつでもノートアプリと組み合わせられます：

- **Obsidian**：フォルダを vault として開きます——`[[wikilinks]]` がクリック可能になり、
  Properties UI が付き、ナレッジマップが本物の canvas として描画されます。
  ゼロ移行・ゼロ config 変更です。
- **Heptabase**：Heptabase で執筆する（`backends: ["heptabase"]`）か、`backends: ["heptabase", "local"]` で
  Heptabase とフォルダの間の完全なブロック単位の**双方向同期**を手に入れます——
  書き戻し（write-back）、手書きの .md ファイルの取り込み（adoption）、コンフリクト台帳、
  プロパティの三方向同期付き。
- **HackMD**（公開ファースト）：`note-sync` の hackmd セグメントが選んだ collection を、
  本物のノート間リンクつきの HackMD ノートとしてミラーリングします——
  オーバービューを共同研究者と共有するために作られています。オプトインの
  `write_back` で「HackMD 上で自分だけが編集できる」ノートは双方向になります
  （共同編集可能なノートは決して書き戻されません）。

どちらのアプリのセットアップも、二重ストアモード（`backends: ["heptabase", "local"]`）の完全な同期の仕組みも、wiki にあります：
...
- **skill を名指し**——Claude Code ではスラッシュで：`/research-cards:<skill>`（例：
  `/research-cards:note-sync`）。Codex ではメッセージの中で skill 名を挙げるだけで OK です。
...
仕組みの詳細は wiki の [Note App Backends](https://github.com/SungFeng-Huang/research-cards/wiki/Note-App-Backends-ja) を参照。agent は実行後に JSON レポートを読み、コンフリクトと TODO をあなたに展開して見せます。
