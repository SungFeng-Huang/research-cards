...
> 「把 https://arxiv.org/abs/XXXX.XXXXX 做成卡片」
> 「掃一下 scholar inbox 信箱」——讀信、去重、建卡、上 Tasks、路由到總覽卡
> 「用 card-rewrite 把〈某某卡〉重寫成教學格式」——結構升級、事實不動
...
> 「把這篇新論文加進 <topic> 的總覽卡」
> 「<topic> 的覆蓋率如何？有哪些論文還沒進比較卡？」
> 「幫這張總覽卡刷新導讀」
> 「幫 tokenizer 和 spoken 加一條橫向連結」
> 「跑 graph audit」——結構變動後檢查 hub／橫向連結／知識地圖一致性
...
> 「把今天的進度記到 project card」——在專案 repo 的 session 裡說即可；
> agent 會自己解析出對應的卡（marker → registry），沒有卡會問你要不要建
> 「幫這個專案建一張 project card」——建卡＋骨架＋上 tag＋固定對應一步完成
> 「把 project card 的進度整併成 paper 級」——merge 端收攏累積的區塊
> 「用知識圖幫我的 project card 做 research-gap 分析」——overview-graph
> Operation 5：拿你的領域地圖對照專案卡，找出還沒被回答的缺口
...
> 「幫〈這張總覽卡〉匯出 BibTeX」
> 「把 <hub 卡> 底下整個主題的參考文獻都匯出來」（hub → 子總覽卡 → 論文）
...
> 「跑 obsidian-sync」／「先 dry-run 看一下要動哪些卡」
> 「有衝突嗎？帶我看 Sync Conflicts」
