...
| `obsidian-sync` | Heptabase ↔ Obsidian 双方向同期エンジン（`backends` に両ストアが必要。`note-sync` 経由でも単独でも実行可） |
| `hackmd-sync` | 選んだ collection を **HackMD** へ増分ミラーリング（共有／公開）：本物のノート間リンク、宣言的な権限（collection 単位で上書き可能）、オプトインの**双方向同期**（`write_back`）——HackMD 側で「自分だけが編集できる」ノートへの編集は段落単位でマージして書き戻されます。共同編集可能なノートと両側編集はコンフリクト報告のままです |
...
| HackMD ミラーリング（`hackmd-sync`） | `npm install -g @hackmd/hackmd-cli`＋一度だけの `hackmd-cli login`（API トークンは hackmd.io → Settings → API から取得。plugin の config には決して保存されません） |
...
- **Heptabase**：Heptabase で執筆する（`backends: ["heptabase"]`）か、`backends: ["heptabase", "local"]` で
  ...
  プロパティの三方向同期付き。
- **HackMD**（公開ファースト）：`hackmd-sync` が選んだ collection を、
...
| 「obsidian-sync を実行して」 | `/research-cards:note-sync --mode obsidian` |
...
| `obsidian-sync` が実行を拒否する | これは `backends: ["heptabase", "local"]` でのみ意味を持ちます——単一ストアでは同期するものがありません |
